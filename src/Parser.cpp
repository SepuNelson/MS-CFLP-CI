...
namespace {
int leer_entero(ifstream& archivo, const string& campo) {
    long long valor = 0;
    if (!(archivo >> valor)) {
        throw runtime_error("No se pudo leer el campo: " + campo);
    }
    if (valor < 0 || valor > numeric_limits<int>::max()) {
        throw runtime_error("Valor fuera de rango en el campo: " + campo);
    }
    return static_cast<int>(valor);
}

int leer_entero_positivo(ifstream& archivo, const string& campo) {
    int valor = leer_entero(archivo, campo);
    if (valor <= 0) {
        throw runtime_error("El campo debe ser positivo: " + campo);
    }
    return valor;
}
}
...
Instancia leer_instancia(const string& ruta_archivo) {
    ...
    if (!archivo) {
        throw runtime_error("No se pudo abrir la instancia: " + ruta_archivo);
    }
    ...
    instancia.costo_operacion.assign(instancia.cantidad_clientes, vector<int>(instancia.cantidad_bodegas, 0));
    ...
    int cantidad_incompatibilidades = leer_entero(archivo, "cantidad_incompatibilidades");
    set<pair<int, int>> pares_unicos;
    ...
    for (int indice = 0; indice < cantidad_incompatibilidades; ++indice) {
        ...
        if (cliente_uno < 0 || cliente_uno >= instancia.cantidad_clientes || cliente_dos < 0 || cliente_dos >= instancia.cantidad_clientes) {
            throw runtime_error("Incompatibilidad con cliente fuera de rango");
        }

        if (cliente_uno == cliente_dos) {
            throw runtime_error("Un cliente no puede ser incompatible consigo mismo");
        }

        if (cliente_dos < cliente_uno) {
            swap(cliente_uno, cliente_dos);
        }
    ...
    if (archivo >> token_extra) {
        throw runtime_error("La instancia contiene datos extra al final");
    }

    if (!instancia.dimensiones_validas()) {
        throw runtime_error("La instancia tiene dimensiones inconsistentes");
    }
